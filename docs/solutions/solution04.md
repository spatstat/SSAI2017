...
1.  Try the same thing with intensity *λ* = 1.5.
...
1.  Read off the fitted intensity. Check that this is the correct value of the maximum likelihood estimate of the intensity.
...
1.  Fit the Poisson point process models with loglinear intensity (trend formula `~x+y`) and log-quadratic intensity (trend formula `~polynom(x,y,2)`) to the Japanese Pines data.
...
1.  extract the fitted coefficients for these models using `coef`.
...
1.  Plot the fitted model intensity (using `plot(predict(fit))`)
...
1.  perform the Likelihood Ratio Test for the null hypothesis of a loglinear intensity against the alternative of a log-quadratic intensity, using `anova`.
...
1.  Generate 10 simulated realisations of the fitted log-quadratic model, and plot them, using `plot(simulate(fit, nsim=10))` where `fit` is the fitted model.
...
1.  Now type `step(fit2)` and interpret the results.
...
1.  Read off the fitted coefficients and write down the fitted intensity function.
...
1.  Plot the fitted intensity as a colour image.
...
1.  extract the estimated variance-covariance matrix of the coefficient estimates, using `vcov`.
...
1.  Compute and plot the standard error of the intensity estimate (see `help(predict.ppm)`).
