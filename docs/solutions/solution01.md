...
1.  find out how many trees are recorded;
...
1.  find the dimensions of the study region;
...
1.  obtain an estimate of the average intensity of trees (number of trees per unit area).
...
1.  plot the Ponderosa data with the title *Ponderosa Pine Trees* above it;
...
1.  from your reading of the help file, predict what will happen if we type
...
1.  try different values of the argument `chars`, for example, one of the integers 0 to 25, or a letter of the alphabet. (Note the difference between `chars=3` and `chars="+"`, and the difference between `chars=4` and `chars="X"`).
...
1.  create a point pattern `hills` containing these data. Ensure that the unit of length is given its correct name.
