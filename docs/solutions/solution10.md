...
1.  Fit the stationary Strauss model with the chosen interaction distance using
...
1.  Generate a sequence of values equally spaced across this range, for example, if your range of plausible values was \[0.05, 0.3\], then type
...
1.  Construct a data frame, with one column named `r` (matching the argument name of `Strauss`), containing these values. For example
...
1.  Print and plot the object `fitp`.
...
1.  Compare the computed estimate of interaction distance *r* with your guesstimate. Compare the corresponding estimates of the Strauss interaction parameter *γ*.
...
1.  Extract the fitted Gibbs point process model from the object `fitp` as
...
1.  Plot the *L*-function of the data along with the global simulation envelopes from 19 realisations of the fitted model.
...
1.  Fit a stationary Geyer saturation process to `swedishpines`, with the same interaction distance as for the Strauss model computed in Question 2, and trying different values of the saturation parameter `sat = 1, 2, 3` say.
...
1.  Fit the same model with the addition of a log-quadratic trend.

``` r
gfit <- ppm(swedishpines ~ polynom(x, y, 2), Geyer(r = Ropt, sat = 3))
```

1.  Plot the fitted trend and conditional intensity.
...
```
    ...
    ## Stationary Strauss process
    ## 
    ## First order term:  beta = 0.03087869
    ## 
    ## Interaction distance:    6.984333
    ## Fitted interaction parameter gamma:   0.1386123
    ## 
    ## Relevant coefficients:
    ## Interaction 
    ##   -1.976074 
